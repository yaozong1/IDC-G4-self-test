...
/**
  ...
  * in the root directory of th  // 触摸检测（每100ms检查一次）
  static uint32_t lastTouchCheck = 0;
  static uint32_t manual_test_counter = 0;
  
  // 改为实时检测模式 - 每200ms检测一次并显示原始数据
  if(currentTime - lastTouchCheck >= 200) {
    manual_test_counter++;
    
    // 每50次测试(10秒)显示一次详细信息
    if (manual_test_counter % 50 == 1) {
      SEGGER_RTT_printf(0, "\r\n=== Real-time Touch Monitor #%d ===\r\n", manual_test_counter/50 + 1);
      SEGGER_RTT_WriteString(0, "Try touching XW12A keys now...\r\n");
    }
    
    uint16_t test_value = XW12A_ReadKeys();          // 反转后的数据
    uint16_t raw_value = XW12A_ReadRawData();        // 原始数据
    
    // 显示所有原始读取值，包括异常值
    if (manual_test_counter % 10 == 0) { // 每2秒显示一次原始值
      SEGGER_RTT_printf(0, "Raw: 0x%04X, Inv: 0x%04X ", raw_value, test_value);
      
      if (test_value == 0x0000) {
        SEGGER_RTT_WriteString(0, "(Comm Fail)");
      } else if (test_value == 0x0FFF) {
        SEGGER_RTT_WriteString(0, "(All Keys/Error)");
      } else if (test_value != 0x0FFF && test_value != 0x0000) {
        SEGGER_RTT_WriteString(0, "(Valid - Keys: ");
        for (uint8_t i = 0; i < 12; i++) {
          if (test_value & (1u << i)) {
            SEGGER_RTT_printf(0, "K%u ", i+1);
          }
        }
    ...
    // 如果检测到有效的非零数据，立即报告
    if (test_value != 0x0000 && test_value != 0x0FFF && test_value != 0xFFFF) {
      SEGGER_RTT_printf(0, "TOUCH DETECTED! Raw=0x%04X, Keys: ", test_value);
      for (uint8_t i = 0; i < 12; i++) {
        if (test_value & (1u << i)) {
          SEGGER_RTT_printf(0, "K%u ", i+1);
        }
